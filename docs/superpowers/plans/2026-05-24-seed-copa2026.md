# Seed Copa 2026 — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Atualizar o seed do banco de dados com estádios, seleções e jogos reais da Copa 2026, conforme dados da API Globo Esporte, e renomear os SVGs de bandeiras para usar as siglas da API.

**Architecture:** Dois arquivos modificados — `seed.ts` reescrito com delete+create em vez de upsert para dados de referência, e 23 SVGs de bandeira renomeados via `git mv`. Todos os dados dos 72 jogos são hardcoded no seed (sem chamada de API em runtime).

**Tech Stack:** Prisma 5, TypeScript, PostgreSQL, Node.js, git
...
| Arquivo | Ação |
|---------|------|
| `apps/frontend/public/flags/*.svg` (23 arquivos) | Renomear com `git mv` |
| `apps/backend/prisma/seed.ts` | Reescrever completamente |

---

### Task 1: Renomear 23 arquivos SVG de bandeiras

**Files:**
- Modify: `apps/frontend/public/flags/` (23 renomeações)

- [ ] **Step 1: Executar os 23 `git mv` de uma vez**

Na raiz do repositório `C:\workspace\bolao-trovao`:

```bash
cd apps/frontend/public/flags
git mv ALG.svg AGL.svg
git mv BIH.svg BOS.svg
git mv CIV.svg CDM.svg
git mv COD.svg RDC.svg
git mv CPV.svg CAB.svg
git mv CUW.svg CUR.svg
git mv CZE.svg TCH.svg
git mv ECU.svg EQU.svg
git mv EGY.svg EGI.svg
git mv ENG.svg ING.svg
git mv GER.svg ALE.svg
git mv GHA.svg GAN.svg
git mv IRN.svg IRA.svg
git mv JPN.svg JAP.svg
git mv KOR.svg COR.svg
git mv KSA.svg ARS.svg
git mv NED.svg HOL.svg
git mv NZL.svg NZE.svg
git mv QAT.svg CAT.svg
git mv RSA.svg AFS.svg
git mv SCO.svg ESC.svg
git mv SWE.svg SUE.svg
git mv USA.svg EUA.svg
```

- [ ] **Step 2: Verificar que os 48 arquivos esperados existem**

```bash
ls apps/frontend/public/flags/ | sort
```

Resultado esperado — 48 arquivos, nenhum com código FIFA antigo (GER, NED, ENG, KOR, etc.):
```
AFS.svg  AGL.svg  ALE.svg  ARG.svg  ARS.svg  AUS.svg  AUT.svg
BEL.svg  BOS.svg  BRA.svg  CAB.svg  CAN.svg  CAT.svg  CDM.svg
COL.svg  COR.svg  CRO.svg  CUR.svg  EGI.svg  ESP.svg  ESC.svg
EQU.svg  EUA.svg  FRA.svg  GAN.svg  HAI.svg  HOL.svg  ING.svg
IRA.svg  IRQ.svg  JAP.svg  JOR.svg  MAR.svg  MEX.svg  NOR.svg
NOR.svg  NZE.svg  PAN.svg  PAR.svg  POR.svg  RDC.svg  SEN.svg
SUI.svg  SUE.svg  TCH.svg  TUN.svg  TUR.svg  UZB.svg  URU.svg
```
...
git commit -m "feat: renomear bandeiras SVG de códigos FIFA para siglas da API Globo"
```

---

### Task 2: Reescrever seed.ts

**Files:**
- Modify: `apps/backend/prisma/seed.ts`

- [ ] **Step 1: Substituir o conteúdo completo de `apps/backend/prisma/seed.ts`**
...
Na raiz do projeto (`C:\workspace\bolao-trovao`):
...
Se aparecer erro em `JogoFase`, confirme que está no import da linha 1.  
Se aparecer erro de tipo no `grupo` (`string` vs `Char(1)`), adicione um cast: `grupo: j.grupo as string`.

- [ ] **Step 3: Rodar o seed em banco limpo e verificar contagens**
...
Seed concluído: 16 estádios, 48 seleções, 72 jogos.

🌱  The seed command has been executed.
```

Se o seed não rodar automaticamente após o `migrate reset`, execute manualmente:

```bash
npx prisma db seed
```

- [ ] **Step 4: Confirmar contagens no banco**
...
- `estadio`: 16 registros
- `selecao`: 48 registros — nenhum com código FIFA antigo (`GER`, `NED`, `KOR`…)
- `jogo`: 72 registros — todos com `fase = GRUPOS`

Ou via `psql` / cliente SQL:

```sql
SELECT 'estadio' AS tabela, COUNT(*) FROM estadio
UNION ALL
...
git commit -m "feat: atualizar seed com estádios, seleções e jogos reais da Copa 2026"
```
...
- [ ] `git log --oneline -3` mostra os dois commits desta feature (bandeiras + seed)
- [ ] `ls apps/frontend/public/flags/ | grep -E "^(GER|NED|ENG|KOR|BIH|QAT|IRN|NZL|RSA|CZE|ECU|SWE|CIV|CUW|JPN|GHA|KSA|CPV|COD|SCO|EGY|ALG|USA)\.svg$"` → nenhum resultado (todos os antigos foram renomeados)
- [ ] O frontend exibe bandeiras corretamente para todos os times (verificar visualmente após `pnpm dev`)
